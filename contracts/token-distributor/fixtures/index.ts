...
export async function tokenDistributorFixture() {
  ...
  // @ts-ignore
  await ldt.connect(vault).transfer(owner, 4_500_000_000n);

  const tokenDistributorFactory = new hre.ethers.ContractFactory<[string], TokenDistributor>(TokenDistributorArtifact.abi, TokenDistributorArtifact.bytecode, owner);
