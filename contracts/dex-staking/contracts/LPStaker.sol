...
contract LPStakerV2 is Ownable2Step {
    ...
    function harvest() public {
        ...
        if (staker.pendingReward1 > 0) {
            rewardToken1.safeTransfer(msg.sender, staker.pendingReward1);
            staker.pendingReward1 = 0;
        }

        if (staker.pendingReward2 > 0) {
            rewardToken2.safeTransfer(msg.sender, staker.pendingReward2);
            staker.pendingReward2 = 0;
        }

        emit Harvest(msg.sender, staker.pendingReward1, staker.pendingReward2);
    }

    function distributeRewards(uint256 rewardAmount1, uint256 rewardAmount2) public onlyOwner {

        uint256 rewardPerShare1 = (rewardAmount1 * 1e18) / totalStaked;
        uint256 rewardPerShare2 = (rewardAmount2 * 1e18) / totalStaked;
        ...
        for (uint256 i = 0; i < stakerAddresses.length; i++) {
            ...
            uint256 pendingReward1 = (stakers[stakerAddress].amount * rewardPerShare1) / 1e18;
            uint256 pendingReward2 = (stakers[stakerAddress].amount * rewardPerShare2) / 1e18;

            tr1 += pendingReward1;
            tr2 += pendingReward2;

            if (pendingReward1 > 0) {
                stakers[stakerAddress].pendingReward1 += pendingReward1;
            }
            if (pendingReward2 > 0) {
                stakers[stakerAddress].pendingReward2 += pendingReward2;
            }
        }
...
contract LPStakerV3 is Ownable2Step {
    ...
    function harvest() public {
        ...
        while (round < rewardRounds.length && gasleft() > gasLimit) {
            pendingReward1 += (stakers[msg.sender].amount * rewardRounds[round][0]) / 1e18;
            pendingReward2 += (stakers[msg.sender].amount * rewardRounds[round][1]) / 1e18;
    ...
    function pendingRewards(address _address) public view returns (uint256, uint256) {
        ...
        while (round < rewardRounds.length) {
            pendingReward1 += (stakers[_address].amount * rewardRounds[round][0]) / 1e18;
            pendingReward2 += (stakers[_address].amount * rewardRounds[round][1]) / 1e18;
    ...
    function distributeRewards(uint256 rewardAmount1, uint256 rewardAmount2) public onlyOwner {
        uint256 roundRewardPerShare1 = (rewardAmount1 * 1e18) / totalStaked;
        uint256 roundRewardPerShare2 = (rewardAmount2 * 1e18) / totalStaked;
